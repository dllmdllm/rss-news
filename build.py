...
# Force UTF-8 output on Windows. encoding can be None when stdout is a pipe
# without PYTHONIOENCODING set — treat that the same as non-UTF-8.
...
async def main():
    ...
    try:
        ...
        _tlog("embed timed out — keeping previous embeddings")
        mark_step("embed", ok=False, error="timeout 180s", seconds=time.monotonic() - t)
    except Exception as exc:
        _tlog(f"embed: {exc!r}")
